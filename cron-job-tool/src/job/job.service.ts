    ...
    private async startRuntime(job: Job) {
        ...
        if (job.type === 'every') {
            ...
            const ref = setInterval(async () => {
                ...
                try {
                    const result  = this.jobAgentService.runJob(job.instruction);
                    this.logger.log(`run job ${job.id}, ${job.instruction}, result: ${result}`);
                } catch (error) {
                    this.logger.error(`run job ${job.id}, ${job.instruction}, error: ${error}`);
                }
            }, job.everyMs);
