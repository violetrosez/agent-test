...
// ---------- 用户问题 ----------
const question = "东东和光光未来各自从事什么职业？";

// ---------- 向量存储与检索 ----------
// 用嵌入模型把文档转成向量并存入内存向量库
const vectorStore = await MemoryVectorStore.fromDocuments(documents, embeddings);
// 构造检索器：按问题检索最相关的 k 条
const retriever = vectorStore.asRetriever({ k: 3 });
const docs = await retriever.invoke(question);

// 可选：带相似度分数的检索结果（用于调试或展示分数）
// const scoredResults = await vectorStore.similaritySearchWithScore(question, 3);
// console.log(scoredResults);

// docs.forEach((doc, i) => {
//     const res = scoredResults.find(score => score[0].pageContent === doc.pageContent);
//     const score = res[1];
...
// ---------- 组装上下文：把检索到的片段拼成给模型的文本 ----------
const context = docs
    .map((doc, i) => `[片段${i + 1}] ${doc.pageContent}`)
    .join("\n");

// ---------- 构造 RAG 提示：系统角色 + 故事片段 + 问题 ----------
