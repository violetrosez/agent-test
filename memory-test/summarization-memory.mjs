/**
 * 总结策略记忆：当对话 token 数超过阈值时，用大模型总结旧消息，保留最近消息 + 总结
 * 适用场景：长对话需要压缩历史以节省上下文窗口
 */
 ...
 * @param {Object} encoder - tiktoken 编码器
...
async function summarizationMemoryDemo() {
    ...
    if (totalTokens >= maxTokens) {
        // 从后往前累加，保留最近消息直到达到 keepRecentTokens
...
/**
 * 调用大模型总结历史对话
 * @param {Array} messages - 需要总结的消息数组
 * @returns {Promise<string>} 总结文本
 */
async function summarizeHistory(messages) {
    if (messages.length === 0) return "";

    // 将消息数组转为可读文本（用户/助手 前缀）
